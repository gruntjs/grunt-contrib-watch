...
exports.fail = {
  ...
  warn: function(test) {
    ...
    assertWatch([function() {
      ...
      test.ok(result.match(/This task should warn/g).length === 2, 'grunt.warn should not stop the chokidar task.');
      test.done();
    });
  },
  fatal: function(test) {
    ...
    assertWatch([function() {
      ...
      helper.verboseLog(result);
      test.ok(result.match(/This task should be fatal/g).length === 2,
        'grunt.fatal should not stop the chokidar task.');
      test.done();
    });
  },
};
